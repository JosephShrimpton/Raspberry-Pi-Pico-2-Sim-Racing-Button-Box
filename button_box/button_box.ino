...
// Encoders: track last CLK pin state to detect rotation, plus a timer
// to auto-release the "click" button after ENCODER_PULSE_MS.
struct EncoderState {
  int clkPin;
  int dtPin;
  int buttonUp;
...
EncoderState encoders[] = {
  { ENC1_CLK, ENC1_DT, 1, 2, HIGH, 0, 0 },
  { ENC2_CLK, ENC2_DT, 4, 5, HIGH, 0, 0 },
  { ENC3_CLK, ENC3_DT, 7, 8, HIGH, 0, 0 },
};
...
struct LedState {
  int pin;
  bool on;         // steady on/off target
  bool flashing;    // if true, ignore "on" and blink instead
  unsigned long flashIntervalMs;
  unsigned long lastToggleTime;
  bool flashPhase;  // current on/off phase while flashing
};

LedState leds[] = {
  { BTN1_LED, false, false, 300, 0, false },
  { BTN2_LED, false, false, 300, 0, false },
  { BTN3_LED, false, false, 300, 0, false },
  { BTN4_LED, false, false, 300, 0, false },
};
...
// ---------------------------------------------------------------------
// Reads all 3 rotary encoders. Each detent turn fires a short button
// press on either the "up" or "down" joystick button, which then
// auto-releases itself after ENCODER_PULSE_MS.
// ---------------------------------------------------------------------
void readEncoders(unsigned long now) {
  for (int i = 0; i < NUM_ENCODERS; i++) {
    int clkState = digitalRead(encoders[i].clkPin);

    if (clkState != encoders[i].lastClkState) {
      // CLK just changed — check DT to work out direction
      int dtState = digitalRead(encoders[i].dtPin);

      if (clkState == LOW) { // only act on the falling edge of CLK
        if (dtState != clkState) {
          // Turned one direction ("up")
          Joystick.button(encoders[i].buttonUp, true);
          encoders[i].upReleaseTime = now + ENCODER_PULSE_MS;
        } else {
          // Turned the other direction ("down")
          Joystick.button(encoders[i].buttonDown, true);
          encoders[i].downReleaseTime = now + ENCODER_PULSE_MS;
        }
      }
      encoders[i].lastClkState = clkState;
    }
...
//   LED1_ON
//   LED1_OFF
//   LED1_FLASH_300      (flash with 300ms on/off interval)
//   LED2_ON
//   LED3_FLASH_150
//   LED4_OFF
//
...
void updateDisplay(String line) {
  ...
  int lineNum = 0;
  while (remaining.length() > 0 && lineNum < 3) {
...
// ---------------------------------------------------------------------
// Handles the actual on/off blinking for any LED currently in flash mode.
// ---------------------------------------------------------------------
void updateLeds(unsigned long now) {
  for (int i = 0; i < NUM_LEDS; i++) {
    if (!leds[i].flashing) continue;

    if (now - leds[i].lastToggleTime >= leds[i].flashIntervalMs) {
      leds[i].flashPhase = !leds[i].flashPhase;
      digitalWrite(leds[i].pin, leds[i].flashPhase ? HIGH : LOW);
